...
func (p *actorChild) Call(childID, funcName, traceId string, args any) {
	if childActor, found := p.Get(childID); found {
		path := cfacade.NewChildPath("", p.thisActor.ActorID(), childID)
		childActor.Call(path, funcName, traceId, args)
	}
}

func (p *actorChild) CallWait(childID, funcName, traceId string, arg, reply any) int32 {
	if childActor, found := p.Get(childID); found {
		path := cfacade.NewChildPath("", p.thisActor.ActorID(), childID)
		return childActor.CallWait(path, funcName, traceId, arg, reply)
	}
