...
type (
	IActorSystem interface {
		...
		Call(source, target, funcName, traceId string, arg any) int32
		CallWait(source, target, funcName, traceId string, arg, reply any) int32
		CallType(nodeType, actorID, funcName, traceId string, arg any) int32
	...
	IActor interface {
		...
		ActorID() string
		Path() *ActorPath
		Call(targetPath, funcName, traceId string, arg any) int32
		CallWait(targetPath, funcName, traceId string, arg, reply any) int32
		CallType(nodeType, actorID, funcName, traceId string, arg any) int32
	...
	IActorChild interface {
		...
		Call(childID, funcName, traceId string, arg any)                     // 调用当前子actor的函数
		CallWait(targetPath, funcName, traceId string, arg, reply any) int32 // 调用当前子actor的函数并等待返回
	}
)
