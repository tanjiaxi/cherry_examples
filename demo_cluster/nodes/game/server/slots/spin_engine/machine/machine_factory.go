 ...
 * @return IMachine
 */
type MachineFactoryFunc func(roomId int32, session *cproto.Session, roomDataInfo *spinManager.RoomDataInfo, userInfo *pb.GetUserInfoResponse) IMachine
...
func CreateMachineByType(ruleId int32, roomId int32, session *cproto.Session, roomDataInfo *spinManager.RoomDataInfo, userInfo *pb.GetUserInfoResponse) IMachine {
	if factoryFunc, ok := machineRegistry[ruleId]; ok {
...
func RegisterMachineAll() {
	//rule：1
	RegisterMachineFactory(1, func(roomId int32, session *cproto.Session, roomDataInfo *spinManager.RoomDataInfo, userInfo *pb.GetUserInfoResponse) IMachine {
		baseMachine := NewBaseMachine(roomId, session, roomDataInfo, userInfo, roomId%1000)
		return NewMachineInfo1(*baseMachine)
	})
	//ruel:2
	RegisterMachineFactory(2, func(roomId int32, session *cproto.Session, roomDataInfo *spinManager.RoomDataInfo, userInfo *pb.GetUserInfoResponse) IMachine {
		baseMachine := NewBaseMachine(roomId, session, roomDataInfo, userInfo, roomId%1000)
